...
def create_tables():
    try:
        ...
        for fname in ('schemas.sql', 'procedures.sql'):
...
def insert_console():
    ...
    try:
        ...
        group_id = _get_or_create_group(cur, group) if group else None #getting group id
        ...
        _add_phones_interactive(cur, contact_id) #add 1 or more phones
...
def _get_or_create_group(cur, group_name: str) -> int: #returns group id, creates if not exists
...
def insert_csv(filepath: str): #csv: name, phone, phone_type, email, birthday, group
    try:
        conn = get_connection()
        cur  = conn.cursor()
...
def import_json(filepath: str = 'contacts.json'):
    ...
    try:
        ...
        for c in data:
            ...
            cur.execute('SELECT id FROM contacts WHERE name = %s;', (name,)) #check copy
...
def main():
    ...
    while True:
        ...
        elif n == '2': insert_csv(r'contacts.csv')
        elif n == '3': update()
